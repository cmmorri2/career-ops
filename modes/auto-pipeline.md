...
Always produce both durable artifacts for a processed application:
- source HTML under `output/` or the matching `output/{application-slug}/` package directory
- rendered PDF in the same directory as that source HTML

Do not write the recorded source HTML to `/tmp`, and do not skip HTML/PDF generation based on score.
